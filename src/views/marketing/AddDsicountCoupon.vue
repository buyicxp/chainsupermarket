<template>
	<div class="form">
		<el-form ref="form" :model="form" label-width="110px">
			<el-form-item label="优惠券名称:"><el-input v-model="form.couponName" placeholder="请输入优惠券名称" style="width: 40%;"></el-input></el-form-item>
			<el-form-item label="优惠券类型:">
				<template>
					<el-select v-model="form.couponType.couponTypeId" style="width: 12%;" clearable placeholder="请选择">
						<el-option v-for="item in CouponTypeList" :key="item.couponTypeId" :label="item.couponTypeName" :value="item.couponTypeId"></el-option>
					</el-select>
				</template>
			</el-form-item>
			<el-form-item label="面值:"><el-input v-model="form.faceValue" style="width: 9%;"></el-input></el-form-item>
			<el-form-item label="发放类型:">
				<template>
					<el-select v-model="form.distributionType.distributionId" style="width: 20%;" clearable placeholder="请选择">
						<el-option v-for="item in DistributionTypeList" :key="item.distributionId" :label="item.distributionName" :value="item.distributionId"></el-option>
					</el-select>
				</template>
			</el-form-item>

			<el-form-item label="每人限领:">
				<el-input v-model="form.limitGet" style="width: 5%;"></el-input>
				张
			</el-form-item>

			<el-form-item label="发放总量:">
				<el-input v-model="form.paymentAmount" style="width: 5%;"></el-input>
				张
			</el-form-item>

			<el-form-item label="已发放:">
				<el-input v-model="form.couponCount" style="width: 5%;" :disabled="true"></el-input>
				张
			</el-form-item>
			...
			<el-form-item label="有效期:">
				<div class="radio">
					<el-radio v-model="form.periodOfValidity" :label="0">
						<span class="radiotext">自领取后</span>
						<el-input v-model="form.effectiveDays" style="width: 50px;"></el-input>
						<span class="radiotext">天内有效</span>
					</el-radio>
				</div>
				<div class="radio">
					<el-radio v-model="form.periodOfValidity" :label="1">
						<span class="radiotext">自领取后</span>
						<el-input v-model="form.effectivePeriod" style="width: 50px;"></el-input>
						<span class="radiotext">天内生效， 生效后</span>
						<el-input v-model="form.effectiveDays" style="width: 50px;"></el-input>
						<span class="radiotext">天失效</span>
					</el-radio>
				</div>
				<div class="radio">
			...
			<el-form-item label="计算时间方式:">
				<template>
					<el-select v-model="form.ComputingTime.computingId" style="width: 20%;" clearable placeholder="请选择">
						<el-option v-for="item in ComputingTimeList" :key="item.computingId" :label="item.computingName" :value="item.computingId"></el-option>
					</el-select>
				</template>
			</el-form-item>
			...
			<el-form-item label="使用限制:">
				<template>
					<el-select v-model="UsageRestriction.restrictionName" style="width: 10%;" clearable placeholder="请选择">
						<el-option v-for="item in UsageRestrictionList" :key="item.value" :label="item.value" :value="item.value"></el-option>
					</el-select>
					<span class="usage">
						<el-time-select
							placeholder="起始时间"
							style="width: 13%;"
							v-model="UsageRestriction.startTime"
					...
					<span class="usage">
						<el-time-select
							placeholder="结束时间"
							style="width: 13%;"
							v-model="UsageRestriction.stoptime"
							:picker-options="{ start: '08:30', step: '00:15', end: '18:30', minTime: startTim }"
						></el-time-select>
					</span>
				</template>
			</el-form-item>
			...
			<el-form-item label="不可用类型:">
				<template>
					<el-select v-model="UnavailableType.unavailableId" style="width: 20%;" clearable placeholder="请选择">
						<el-option v-for="item in CommdityTypeList" :key="item.id" :label="item.typename" :value="item.id"></el-option>
					</el-select>
				</template>
			</el-form-item>
			...
			<el-form-item label="指定门店:">
				<template>
					<el-select v-model="form.store.spId" style="width: 20%;" clearable placeholder="请选择">
						<el-option v-for="item in storeList" :key="item.spId" :label="item.spName" :value="item.spId"></el-option>
					</el-select>
				</template>
			</el-form-item>

			<el-form-item label="订单金额:"><el-input v-model="form.money" style="width: 5%;"></el-input></el-form-item>

			<el-form-item label="优惠类型:">
				<template>
					<el-select v-model="form.PreferentialType.preferentialId" style="width: 20%;" clearable placeholder="请选择">
						<el-option v-for="item in PreferentialTypeList" :key="item.preferentialId" :label="item.preferentialName" :value="item.preferentialId"></el-option>
					</el-select>
				</template>
			</el-form-item>
...
export default {
	...
	data() {
		return {
			...
			ComputingTimeList: [],
			//使用限制
			UsageRestrictionList: [{ value: '星期一' }, { value: '星期二' }, { value: '星期三' }, { value: '星期四' }, { value: '星期五' }, { value: '星期六' }, { value: '星期日' }],
			//商品的所有分类
			CommdityTypeList: [],
			//门店
			storeList: [],
	...
	methods: {
			...
			if (that.verify) {
				...
				that.$axios
					...
					.then(response => {
						...
						this.$router.replace('/DsicountCouponInfo');
					})
		...
		checks() {
			...
			if (that.UsageRestriction.restrictionName != null && that.UsageRestriction.restrictionName != '') {
				if (that.UsageRestriction.startTime == null || that.UsageRestriction.stoptime == null || that.UsageRestriction.startTime == '' || that.UsageRestriction.stoptime == '') {
		...
		selectDiscountCoupon() {
			var that = this;
			console.log(localStorage.getItem('discountcoupon'));
			//判断有是什么操作，有值就是更新操作，没值就是添加操作
			if (localStorage.getItem('discountcoupon') == null) {
				that.verify = false;
				return;
			}
			that.couponId = localStorage.getItem('discountcoupon');
			console.log(that.couponId);
			let url = 'http://localhost:8080/discount/findDiscountCoupon?couponId=' + localStorage.getItem('discountcoupon');
			// console.log(url)
			console.log('查询优惠券');
			this.$axios
				.get(url)
			...
			let urlOne = 'http://localhost:8080/discount/usagerestriction?couponId=' + localStorage.getItem('discountcoupon');
			// console.log(url)
			console.log('查询使用限制');
			this.$axios
				.get(urlOne)
			...
			let urlTwo = 'http://localhost:8080/discount/unavailabletype?couponId=' + localStorage.getItem('discountcoupon');
			// console.log(url)
			console.log('查询使用限制');
			this.$axios
				.get(urlTwo)
